...
export default function CashFlowCard({ companyId, companyLabel }: { companyId: string; companyLabel?: string }) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<{ cashIn: number; cashOut: number; realCashFlow: number } | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const json = (await res.json()) as { cashIn: number; cashOut: number; realCashFlow: number };
  ...
  return (
    <div className="rounded-3xl border border-surface-800 bg-surface-900/70 p-6 shadow-xl shadow-black/20">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-surface-400">Cash Flow</p>
          <h3 className="mt-2 text-lg font-semibold text-white">
            {companyLabel ?? "Selected Company"}
          </h3>
          <p className="mt-1 text-sm text-surface-400">Collections vs cash out (MVP)</p>
        </div>

        <div className="rounded-2xl border border-surface-800 bg-surface-950/70 px-4 py-2">
          <p className="text-xs uppercase tracking-[0.18em] text-surface-400">Real Cash Flow</p>
          <p
            className={[
      ...
      {error ? (
        <div className="mt-4 rounded-2xl border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-200">
          <p className="font-semibold">Couldn&rsquo;t load cash flow</p>
