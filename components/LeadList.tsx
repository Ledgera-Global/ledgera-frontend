...
export default function LeadList({
  ...
  return (
    <div>
      <div className="mb-4">
        <p className="text-xs uppercase tracking-[0.24em] text-surface-400">Booked Leads</p>
        ...
        {leads.map((lead) => {
          ...
          return (
            <button
              ...
              className={[
                "w-full rounded-2xl border p-4 text-left transition hover:border-surface-600 hover:bg-surface-900",
                isSelected
                  ? "border-brand-500/40 bg-brand-500/10"
                  : "border-surface-800 bg-surface-950/70",
              ].join(" ")}
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-white">{lead.company}</h3>
                  <p className="text-sm text-surface-400">{lead.revenue}</p>
                  <p className="text-sm text-surface-300">{lead.techs} techs</p>
                </div>
              ...
              <div className="mt-3 h-2 w-full rounded-full bg-surface-800">
                <div
                  className={`h-2 rounded-full bg-gradient-to-r from-brand-400 to-brand-500 ${scoreFillClass(
                    lead.score
                  )}`}
                />
              </div>

              <p className="mt-2 text-xs text-surface-500">Close probability {lead.score}%</p>
            </button>
          );
        })}
      </div>
